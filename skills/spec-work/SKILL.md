...
**스킬 재호출 시 (승인 게이트 확인 등).** 이 스킬 본문이 이미 이 세션 컨텍스트에 있는 상태에서 `spec_work` 를 다시 호출할 때는 `skill_loaded=true` 로 호출한다. (예: 승인 후 구현 재진입, 다음 todo 진행) 서버가 스킬 본문을 생략하고 게이트 판정만 반환하므로 토큰이 절약된다.
...
**E2E 항목 선택 시 순서 확인 (필수)**

선택된 항목이 `T-NNE` 형식(E2E 테스트)인 경우, 대응하는 구현 항목(`T-NN`)이 완료(`[x]`) 상태인지 확인한다.
완료되지 않은 경우 진행을 중단하고 사용자에게 알린다.

```
⚠️ T-NNE 는 T-NN 구현이 완료된 후 진행해야 합니다.
현재 T-NN 상태: [ ] TODO / [ ] IN PROGRESS

먼저 T-NN 을 완료한 후 T-NNE 를 진행하세요.
```
...
3. 모듈 간 의존 관계가 변경된 경우 `index.md` 의 "모듈 의존 관계" 표(SSOT)만 갱신한다. (`modules/<domain>.md` 에는 module→module 엣지를 적지 않는다)
4. 신규 파일·디렉토리가 생긴 경우 `index.md` 모듈 맵과 디렉토리 구조도 갱신한다.
5. 구현 중 코드에서 직접 확인된 암묵적 제약·주의사항이 있으면 `ai-spec/_codebase/gotchas.md` 에 추가한다. (rules의 `gotchas.md 포맷` 섹션 작성 기준 참조)
6. `ai-spec/_codebase/last-synced.md` 를 갱신한다. (일시, `git rev-parse HEAD`, 트리거: `spec-work T-NN`, 업데이트 모듈 목록)
