...
argument-hint: "feature 폴더명 또는 todo 항목 번호 (예: seller-dashboard, T-01). 생략 시 AI가 todo.md 를 확인하여 다음 작업을 제안한다."
---
...
대상 todo 가 결정되면 **vscode_askQuestions 도구**로 확인을 요청한다.
...
**search.md 탐색 규칙**
- `search.md` 가 없거나 해당 todo 구현에 필요한 코드 위치 정보가 부족한 경우
  워크스페이스를 가볍게 탐색하여 필요한 정보를 수집한다.
- 탐색 결과는 `PROJECT_SPEC_DIR/search.md` 에 추가 기록한다.
...
예시: `ai-spec/projects/seller-dashboard/T-01-orderSVC-getList/plan.md`
...
**vscode_askQuestions 도구**로 아래 질문을 표시하고 사용자 응답을 기다린다.
...
- 구현 중 추가로 파악된 코드 위치·스키마는 `PROJECT_SPEC_DIR/search.md` 에 추가
...
todo.md 에서 미완료 항목이 있는 경우 **vscode_askQuestions 도구**로 다음 진행 여부를 묻는다.
...
- 구현 중 발견된 모든 코드 위치·스키마 정보는 `search.md` 에 누적 기록한다.
