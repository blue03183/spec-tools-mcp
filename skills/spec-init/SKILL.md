...
`_codebase/` 초기화(신규 생성)가 발생한 경우에만 실행한다. 이미 `_codebase/`가 있던 경우(갱신/최신 상태)는 건너뛴다.
...
`.claude/settings.json`을 확인하여 `PreToolUse` 항목이 없으면 생성하거나 `hooks` 키를 병합한다.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cat | python3 -c \"import json,sys; d=json.load(sys.stdin); f=d.get('tool_input',{}).get('file_path',''); f and f.endswith('.ts') and 'ai-spec' not in f and print('[코드베이스] 소스 파일 탐색 전 ai-spec/_codebase/index.md 를 먼저 확인하세요 — 모듈 상세: ai-spec/_codebase/modules/')\" 2>/dev/null; exit 0"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cat | python3 -c \"import json,sys; d=json.load(sys.stdin); f=d.get('tool_input',{}).get('file_path',''); f and 'ai-spec' not in f and print('[위키 동기화] 소스 파일 변경됨 → ai-spec/_codebase/ 업데이트 필요한지 확인하세요.')\" 2>/dev/null; exit 0"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cat | python3 -c \"import json,sys; d=json.load(sys.stdin); f=d.get('tool_input',{}).get('file_path',''); f and 'ai-spec' not in f and print('[위키 동기화] 새 소스 파일 생성됨 → ai-spec/_codebase/ 위키 추가 필요한지 확인하세요.')\" 2>/dev/null; exit 0"
          }
        ]
      }
    ]
