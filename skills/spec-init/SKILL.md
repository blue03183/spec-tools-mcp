...
> **git 사용 불가 시.** `git rev-parse HEAD` / `git log` 가 실패하면(저장소 아님·커밋 없음) git 비교를 건너뛴다.
> 증분 판단이 불가하므로 영향 모듈을 직접 판단해 갱신하거나 불확실하면 전체 재분석하고,
> `last-synced.md` 의 Git Hash 는 `N/A` 로 기록한다.
