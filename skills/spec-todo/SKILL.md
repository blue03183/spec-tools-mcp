...
requirement.md 가 없거나 템플릿 상태인 경우, 새로 생성하기 전에 커스텀 템플릿 존재 여부를 확인한다.
...
선택지:
  - 복잡 (신규 API, DB 스키마 변경, 컴포넌트 아키텍처 등) → design.md 작성 후 todo.md 생성
  - 단순 (기존 패턴 확장, UI 수정 등) → design.md 생략, todo.md 바로 작성
```

**"복잡" 선택 시 — design.md 초안 작성**

1. `requirement.md` 를 읽어 기능 목표·사용자 시나리오·승인 기준을 분석한다.
2. 분석 내용을 바탕으로 `ai-spec/projects/<feature>/design.md` 초안을 AI가 직접 작성한다.
...
3. 저장 완료 후 **질문 도구**로 검토를 요청한다.
   ...
   ```
   질문: design.md 초안이 저장되었습니다. (ai-spec/projects/<feature>/design.md)
