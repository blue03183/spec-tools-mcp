...
### A-6 — 아키텍처 문서(architecture.md) 생성 여부 확인

requirement.md 작성 완료 후, Procedure B(todo.md 생성)로 넘어가기 전에
**질문 도구**로 feature 복잡도를 확인한다.

```
질문: 이 기능의 복잡도를 선택해주세요.

선택지:
  - 복잡 (신규 API, DB 스키마 변경, 컴포넌트 아키텍처 등) → architecture.md 작성 후 todo.md 생성
  - 단순 (기존 패턴 확장, UI 수정 등) → architecture.md 생략, todo.md 바로 작성
```

**"복잡" 선택 시 — architecture.md 초안 작성**

1. `requirement.md` 를 읽어 기능 목표·사용자 시나리오·승인 기준을 분석한다.
2. 분석 내용을 바탕으로 `ai-spec/projects/<feature>/architecture.md` 초안을 AI가 직접 작성한다.
   각 섹션을 실제 내용으로 채운다. 비워두지 않는다.
...
3. 저장 완료 후 **질문 도구**로 검토를 요청한다.

   ```
   질문: architecture.md 초안이 저장되었습니다. (ai-spec/projects/<feature>/architecture.md)
         파일을 열어 내용을 검토하고 필요한 부분을 수정한 후 안내해주세요.
...
- `architecture.md` 가 존재하는 경우, B-2(워크스페이스 탐색) 전에 읽어 탐색 방향에 반영한다.
