...
```
질문: [섹션명] 섹션에 기존 내용과 기획서 분석 내용이 충돌합니다. 어떤 내용으로 업데이트할까요?
      기존 내용: ...
      기획서 분석 내용: ...
...
사용자가 응답하면 A-5 으로 이동한다.

---

### A-5 — UI 미리보기(preview.html) 생성 여부 확인
...
`todo.md` 가 이미 존재하는 경우, requirement.md 의 수정 시각이 todo.md 보다 최신인지 확인한다.
최신인 경우 **질문 도구**로 재생성 여부를 묻는다.

```
질문: requirement.md 가 todo.md 보다 최근에 수정되었습니다. todo.md 를 재생성할까요?

선택지:
  - 재생성 (기존 todo.md 를 덮어씁니다)
  - 유지 (현재 todo.md 를 그대로 사용합니다)
```
...
요구사항이 화면 흐름·API 통합 동작 검증을 요구하는 경우, 대응 구현 항목(`T-NN`)에 `E` 를 붙인 E2E 항목(`T-NNE`)을 함께 생성한다.
단위 로직만 있는 항목에는 만들지 않는다. 상세 기준은 rules의 `Step 4 — todo` 내 `E2E 테스트 항목` 참조.
