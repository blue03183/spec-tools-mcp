...
**VS Code + GitHub Copilot**（`.vscode/mcp.json`）——上記のオプション A または B を使用
...
**`spec_work`**  
実装を開始するとき、または前のセッションを引き継ぐときに使用します。計画 → 承認 → 実装のゲートを強制し、`plan.md` を作成して人間の承認を得るまで実装をブロックします。各ステップの進捗を `update.md` に記録するため、セッションが途切れても正確に中断箇所から再開できます。
