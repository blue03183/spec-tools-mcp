...
[Node.js](https://nodejs.org) v18 以降（LTS 推奨）。MCP サーバーは `npx` で起動され、`spec-init` がインストールするコードベースウィキのコンテキストフックも `node` で動作するため、Node.js が `PATH` に存在する必要があります。`node -v` で確認してください。
...
MCP サーバーと Skills をプラグインとして一括インストールします：
...
> Copilot チャットでスキルを直接呼び出すには、**プラグインインストール**を使用してください。
> MCP サーバーのみをインストールすると、ツール名にプレフィックス（`blu_`）が付き、チャットパネルからスキルを直接呼び出すことができません。

**プラグインとしてインストール**（MCP サーバー + Skills を同時インストール）：
...
<details>
<summary>.vscode/mcp.json を生成する</summary>

自動設定で `.vscode/mcp.json` を生成します：

```bash
npx spec-tools-mcp init
```

VS Code の MCP サーバーはターミナルではなく `VSCode Extension Host` 内で実行されるため、`npx` および `node` コマンドが認識されない場合があります。
`.vscode/mcp.json` に `command` と env `PATH` を明示的に指定してください：
...
```json
{
  "servers": {
    "spec-tools-mcp": {
      "type": "stdio",
      "command": "/Users/{ユーザー名}/.nvm/versions/node/v24.11.0/bin/npx",
      "args": ["-y", "spec-tools-mcp@latest"],
      "env": {
        "PATH": "/Users/{ユーザー名}/.nvm/versions/node/v24.11.0/bin:/usr/local/bin:/usr/bin:/bin"
      }
    }
  }
}
...
> `which npx` で npx のパスを、`echo $PATH` で PATH の値を確認してください。

IDE ウィンドウをリロードした後、**拡張機能** → **MCP サーバー - インストール済み** で `spec-tools-mcp` を右クリック → **サーバーを起動** を選択して手動で起動してください。

> **注意：** IDE ウィンドウをリロードした場合、サーバーを再起動する必要があります（自動では再起動されません）。

</details>

<details>
<summary>MCP サーバーのみインストール（ワンクリック、スキルの直接呼び出し不可）</summary>
...
CLI でインストール：
（Codex CLI がインストールされていない場合は先にインストールしてください：`npm install -g @openai/codex`）
...
> プロジェクト設定が反映されない場合は、`vi ~/.codex/config.toml` でグローバル設定に追加してください。

---

**Kiro**

プロジェクトルートで自動設定（`.kiro` フォルダが既に存在する必要があります）：

```bash
npx spec-tools-mcp init
```

`.kiro` フォルダが存在しない場合は、先に Kiro でプロジェクトを開いてフォルダを作成してから `init` を実行してください。`.kiro/settings/mcp.json` が作成されます。

または `.kiro/settings/mcp.json` を手動で作成します：
...
保存後に Kiro を再起動するか、Kiro の機能パネル（**MCP サーバー → spec-tools-mcp → サーバーを起動**）からサーバーを手動で起動してください。「利用可能な MCP ツールは何ですか？」と質問して接続を確認できます。
...
Claude Code、Cursor、VS Code、Kiro を自動検出し、それぞれの設定ファイルを生成します。

または手動で MCP 設定ファイルに追加：
...
<details>
<summary>ローカルインストールパスを使用する場合</summary>
...
サーバーが接続されていれば、AI が 8 つのツール（`spec_init`、`spec_todo`、`spec_work`、`get_rules`、`spec_status`、`spec_handoff`、`spec_archive`、`spec_search`）を列挙するか、開発ルールドキュメントを返します。
...
**`spec_todo`**  
企画書や要件が揃ったら実行します。`docs/` のファイルを分析して `requirement.md` を作成し、シンプルなタスク一覧（`todo.md`）を生成します。実装前に AI と人間が要件を共同レビューするチェックポイントとして機能します。
...
**`spec_status`**  
複数の機能が並行して進行しているときに全体状況を把握したい場合に使用します。全 feature の todo 完了率、進行中（`IN PROGRESS`）と未着手（`TODO`）の項目を区別して表示し、承認が必要な plan も一目でわかるため、承認リクエストの見落としを防ぎます。

**`spec_handoff`**  
チームメンバーへ作業を引き継ぐ際や、長期間その機能を中断する必要があるときに使用します。機能目標・todo 状況・主要なコード位置を一つの文書にまとめ、次のセッションや別の開発者がコードベースを再調査せずにすぐ作業を引き継げるようにします。

**`spec_archive`**  
機能開発が完全に完了した後に呼び出します。feature フォルダを `ai-spec/archive/` に移動し、`projects/` をアクティブな作業のみの状態に保ちます。未完了の todo がある場合、または同名のアーカイブが既に存在する場合はブロックされます。
...
3. **`spec_todo` を実行**してドキュメントを分析しスペックファイルを生成
   ...
   - UI 変更が含まれる場合、`preview.html` プレビューを生成してブラウザで開き、レビューを求める
   - シンプルなタスク一覧 `todo.md` を生成（T-01、T-02、…）
   - 画面フローや API 統合の検証が必要な項目には E2E 項目（例: `T-01E`）が併せて生成され、対応する実装項目（`T-01`）の完了後にのみ進行する
   - `requirement.md` が既に存在する場合、分析内容は既存の要件の下に追記される

4. **`spec_work` を実行**して各タスクを実装
   - AI が選択したタスクの `plan.md` を作成し、承認を求める
   - 計画ファイルを直接レビュー——修正が必要な場合は `plan.md` の `User Feedback` セクションに内容を記入し、`修正が必要` を選択
   - `承認` または `進めて` と入力すると実装開始
   - 承認ゲートは MCP サーバーレベルで強制 — `plan.md` が `[待機]` 状態の間、`spec_work` は実装手順の代わりにブロック通知を返すため、承認するまでコードは書かれない
...
8. **機能が完了**したらアーカイブして `projects/` を整理
   - `spec_archive` がフォルダを `ai-spec/archive/` へ移動——未完了の todo があるか同名アーカイブが既に存在する場合はブロック
...
│   └─ gotchas.md                  # 暗黙的な制約・注意事項（人間が作成 / コードで検証）
...
└─ projects/
    └─ <feature>                   # 機能ごとのプロジェクトフォルダ
        ...
        ├─ preview.html            # UI プレビュー（UI 変更が含まれる場合に生成）
...
新しいバージョンがリリースされた場合、インストール方法に応じてアップデートします。

**Claude Code**

インストールコマンドを再実行すると、最新バージョンに置き換えられます。
...
**VS Code / GitHub Copilot（プラグイン）**

初回インストールと同じ手順で進めてください。既存のプラグインが最新バージョンに置き換えられます。

1. **Command Palette** を開く（`Cmd+Shift+P` / `Ctrl+Shift+P`）
2. **Chat: Install Plugin From Source** を実行
3. 同じ URL を貼り付け：`https://github.com/blue03183/spec-tools-mcp`

**npx ベースの設定（Codex CLI、Cursor、その他の IDE）**

`npx -y` を使用した設定は、サーバー起動時に自動的に最新バージョンを取得します。手動での作業は不要です。

古いバージョンが引き続き使用される場合は、次のコマンドでキャッシュを更新してください。

```bash
npx --yes spec-tools-mcp@latest
```

**ローカルインストール（`npm install --save-dev`）**

```bash
npm update spec-tools-mcp
```

アップデート後は AI エージェントを再起動してください。
