...
[Node.js](https://nodejs.org) v18 이상 (LTS 권장). MCP 서버는 `npx`로 실행되고, `spec-init`이 설치하는 코드베이스 위키 컨텍스트 훅도 `node`로 동작하므로 Node.js가 `PATH`에 있어야 합니다. `node -v`로 확인하세요.
...
MCP 서버와 Skills를 플러그인으로 한 번에 설치합니다:
...
> Copilot 채팅에서 스킬을 직접 호출하려면 **플러그인 설치**를 사용하세요.
> MCP 서버만 설치하면 툴 이름에 접두어(`blu_`)가 붙고 채팅창에서 스킬을 바로 불러올 수 없습니다.

**플러그인으로 설치** (MCP 서버 + Skills 함께 설치):
...
<details>
<summary>.vscode/mcp.json 생성</summary>

자동 설정으로 `.vscode/mcp.json`을 생성합니다:

```bash
npx spec-tools-mcp init
```

VS Code의 MCP 서버는 터미널이 아닌 `VSCode Extension Host`에서 실행되기 때문에 `npx` 및 `node` 명령어가 인식되지 않을 수 있습니다.
`.vscode/mcp.json`에 `command`와 env `PATH`를 명시적으로 지정하세요:
...
```json
{
  "servers": {
    "spec-tools-mcp": {
      "type": "stdio",
      "command": "/Users/{사용자이름}/.nvm/versions/node/v24.11.0/bin/npx",
      "args": ["-y", "spec-tools-mcp@latest"],
      "env": {
        "PATH": "/Users/{사용자이름}/.nvm/versions/node/v24.11.0/bin:/usr/local/bin:/usr/bin:/bin"
      }
    }
  }
}
...
> `which npx`로 npx 경로를, `echo $PATH`로 PATH 값을 확인하세요.

설정 후 IDE 창을 새로고침하고, `확장` → `MCP 서버 - 설치됨`에서 `spec-tools-mcp` 서버를 우클릭 → **서버 시작**으로 수동 실행하세요.

> **주의:** IDE 창을 새로고침하면 서버를 다시 시작해야 합니다 (자동 재시작되지 않음).

</details>

<details>
<summary>MCP 서버만 설치 (원클릭, 스킬 직접 호출 불가)</summary>
...
CLI로 설치:
(codex CLI 가 설치되어있지 않다면 설치해주세요. `npm install -g @openai/codex`)
...
> 프로젝트 설정이 적용되지 않는 경우 `vi ~/.codex/config.toml`로 전역 설정에 추가하세요.
...
서버가 연결되면 AI가 여덟 가지 도구(`spec_init`, `spec_todo`, `spec_work`, `get_rules`, `spec_status`, `spec_handoff`, `spec_archive`, `spec_search`)를 나열하거나 개발 규칙 문서를 반환합니다.
...
**`spec_todo`**  
기획서나 요구사항이 준비된 후 실행합니다. `docs/` 폴더의 파일을 분석해 `requirement.md`와 심플한 작업 목록(`todo.md`)을 생성하며, 구현 전에 AI와 사람이 요구사항을 공동 검토하는 체크포인트 역할을 합니다.
...
**`spec_status`**  
여러 기능이 동시에 진행 중일 때 전체 현황을 파악하고 싶을 때 사용합니다. 모든 feature의 todo 완료율, 진행 중(`IN PROGRESS`) 항목과 대기(`TODO`) 항목을 구분해 표시하며, 승인이 필요한 plan을 한눈에 보여줘 승인 대기 항목을 놓치지 않도록 합니다.
...
**`spec_archive`**  
기능 개발이 완전히 완료된 후 호출합니다. feature 폴더를 `ai-spec/archive/`로 이동시켜 `projects/`를 활성 작업만 남긴 상태로 유지합니다. 미완료 todo가 있거나 동명 아카이브가 이미 존재하면 차단됩니다.
...
3. **`spec_todo` 실행** — 기획서 분석 및 스펙 파일 생성
   ...
   - UI 변경이 포함된 경우 `preview.html` 미리보기를 생성하고 브라우저에서 열어 검토 요청
   - 심플한 작업 목록 `todo.md` 생성 (T-01, T-02, …)
   - 화면 흐름·API 통합 검증이 필요한 항목은 E2E 항목(예: `T-01E`)이 함께 생성되며, 대응 구현 항목(`T-01`) 완료 후에만 진행됨
   - `requirement.md`가 이미 있으면 분석 내용을 기존 요구사항 아래에 추가

4. **`spec_work` 실행** — 각 작업 구현
   - AI가 선택된 작업의 `plan.md`를 작성하고 승인 요청
   - 계획 파일을 직접 검토 — 수정이 필요하면 `plan.md`의 `User Feedback` 섹션에 내용 입력 후 `수정 필요` 선택
   - `승인` 또는 `진행해`를 입력하면 구현 시작
   - 승인 게이트를 MCP 서버 레벨에서 강제 적용 — `plan.md`가 `[대기]`인 동안 `spec_work`는 구현 절차 대신 차단 안내를 반환하므로 승인 전에는 코드가 작성되지 않음
...
8. **기능 완료** 후 아카이브하여 `projects/` 정리
   - `spec_archive`가 폴더를 `ai-spec/archive/`로 이동 — 미완료 todo가 있거나 동명 아카이브가 이미 존재하면 차단
...
│   └─ gotchas.md                  # 암묵적 제약·주의사항 (사람 작성 / 코드에서 검증된 항목)
...
└─ projects/
    └─ <feature>                   # 기능별 프로젝트 폴더
        ...
        ├─ preview.html            # UI 미리보기 (UI 변경이 포함된 경우 생성)
...
새 버전이 출시된 경우 설치 방식에 따라 업데이트합니다.

**Claude Code**

설치 명령어를 다시 실행하면 최신 버전으로 교체됩니다.
...
**VS Code / GitHub Copilot (플러그인)**

설치 시와 동일하게 진행합니다. 기존 플러그인이 최신 버전으로 교체됩니다.

1. **Command Palette** 열기 (`Cmd+Shift+P` / `Ctrl+Shift+P`)
2. **Chat: Install Plugin From Source** 실행
3. 동일한 URL 붙여넣기: `https://github.com/blue03183/spec-tools-mcp`

**npx 기반 (Codex CLI, Cursor, 기타 IDE)**

`npx -y` 기반 설정은 서버 시작 시 최신 버전을 자동으로 가져옵니다. 별도 작업이 필요 없습니다.

오래된 버전이 계속 사용되는 경우 아래 명령으로 캐시를 갱신하세요.

```bash
npx --yes spec-tools-mcp@latest
```

**로컬 설치 (`npm install --save-dev`)**

```bash
npm update spec-tools-mcp
```

업데이트 후에는 AI 에이전트를 재시작하세요.
