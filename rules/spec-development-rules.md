...
| todo.md | 어떤 작업이 필요한지 파악하기 위한 심플한 작업 목록 |
...
| `_codebase/modules/<domain>.md` | 도메인별 주요 파일, 핵심 API, 패턴, 내부·외부 의존성 |
| `_codebase/conventions.md` | 공통 컨벤션, 네이밍 규칙, 아키텍처 패턴 |
| `_codebase/gotchas.md` | 모듈별 암묵적 제약, 순서 의존성, 주의사항 |
...
`spec_status`·`spec_work`·`spec_handoff`·`spec_archive` 도구는 아래 파일들을 **정규식으로 파싱**한다.
커스텀 템플릿(`ai-spec/templates/`)을 사용하더라도 아래 구조는 **반드시 유지**해야 한다.
구조가 어긋나면 도구가 항목을 인식하지 못해 진행률·승인 게이트·아카이브 차단이 조용히 오작동한다.

## todo.md 항목 계약

- 각 항목은 `## [T-NN] 제목` 형식의 H2 헤딩으로 시작한다. (`T-NN` 또는 E2E 항목 `T-NNE`)
- 같은 항목 블록(다음 `##` 헤딩 전) 안에 `상태` 를 포함하는 라인이 있어야 한다.
- 상태 판정 규칙(대소문자 무시).
  - `[x]` 포함 → 완료
  - `IN PROGRESS` 포함 → 진행 중
  - 그 외(`[ ] TODO`) → 미시작
...
- 파일 어딘가에 `Approval Status` 텍스트를 포함하는 라인이 있어야 한다.
- 그 다음 비어있지 않은 라인에 `[대기]` 또는 `[승인]` 이 포함되어야 한다. (앞뒤 주석은 허용)
- 위 형식을 찾지 못하면 `spec_work` 는 **승인을 안전하게 차단**한다. (구현 절차를 제공하지 않음)
...
- `spec_handoff` 는 `## 기능 목표` 헤딩 아래 내용을 인계 문서의 목표로 추출한다.
- 커스텀 템플릿을 쓰더라도 `## 기능 목표` 헤딩을 유지하면 인계 문서가 완전해진다.
...
`spec-init` 최초 실행 시 생성되며, `spec-work` Step 7 구현 중(즉시) 및 Step 8 완료 시(최종) 영향받은 모듈을 갱신한다.

> **토큰 효율.** 위키가 비대해지면 탐색·검색 비용이 커지므로, 각 파일은 표 중심으로 간결하게 유지한다. 산문 설명보다 "심볼·파일·한 줄 설명" 표를 우선하고, 본문 코드 복사는 지양한다.

> **의존 관계 SSOT.** 모듈 간 의존 관계(module→module 엣지)의 Single Source of Truth 는 `index.md` 의 "모듈 의존 관계" 표 한 곳이다. `modules/<domain>.md` 에는 이 엣지를 다시 표로 적지 않는다. 같은 엣지를 두 곳에서 관리하면 한쪽만 갱신될 때 그래프가 모순되어 LLM 의 관계 파악이 오히려 나빠지기 때문이다. `modules/<domain>.md` 의 "외부 의존성" 표에는 프로젝트 외부(npm 패키지 등) 의존만 기재한다.
...
## 모듈 의존 관계
| 모듈 | 이 모듈이 사용하는 모듈 | 이 모듈을 사용하는 모듈 |
|------|------------------------|------------------------|
...
| 흐름 | 진입점 | 경로 |
|------|--------|------|
| 주문 생성 | `POST /orders` | OrderController → OrderService → PaymentService → OrderRepository |
| 로그인 | `POST /auth/login` | AuthController → AuthService → UserRepository |
...
<!-- 모듈 간 의존 관계(module→module)는 index.md "모듈 의존 관계" 표가 SSOT 다. 여기에 다시 적지 않는다. -->
...
## 아키텍처 패턴
(전체 코드베이스에 공통으로 적용되는 레이어 구조, 패턴, 설계 원칙)

## 공통 유틸
| 심볼 | 파일 | 설명 |
|------|------|------|
| `formatDate` | `src/utils/date.ts` | 날짜 포맷 공통 함수 |
...
> **작성 주체 및 기준.**
> - 사람이 직접 작성·관리하는 것을 원칙으로 한다.
> - 에이전트는 구현 중 코드 실행·테스트·빌드 과정에서 **직접 확인된** 제약만 추가할 수 있다.
>   추가 가능한 예: 트랜잭션 경계 위반으로 테스트 실패, 호출 순서를 바꿨을 때 실제로 오류 발생
>   추가 불가한 예: 코드를 읽고 "이럴 것 같다"고 추론한 제약, 아직 검증되지 않은 가정
> - 코드 분석만으로 암묵적 제약 전체를 도출할 수 없으므로, 확인되지 않은 내용은 절대 추가하지 않는다.
...
## orders
- PaymentService.charge() 완료 확인 전에 Order 상태를 CONFIRMED 로 변경하면 안 된다
- 재고 차감은 반드시 트랜잭션 내부에서만 수행한다

## auth
- JWT 만료 검증은 미들웨어가 처리하므로 서비스 레이어에서 중복 검증하지 않는다
...
> **git 사용 불가 시 폴백.** 아래 규칙은 git 저장소를 전제로 한다.
> `git` 명령이 없거나(저장소 아님·커밋 없음) 실패하면 git hash 비교를 건너뛴다.
> - `spec-init` — 증분 판단이 불가하므로 `_codebase/` 가 있어도 영향 모듈을 직접 판단해 갱신하거나, 불확실하면 전체 재분석한다.
> - `spec-work` — 변경 파일 목록은 git 대신 `update.md` 의 변경 파일 목록을 기준으로 삼는다.
> - `last-synced.md` 의 Git Hash 는 `N/A` 로 기록하고 일시·트리거만 남긴다.
...
### spec-work Step 8 완료 시
- `update.md` 변경 파일 목록 기준으로 영향받는 모듈 파악
- 해당 `modules/<domain>.md` 업데이트 (신규 심볼, 변경된 API, 패턴 반영)
- 모듈 간 의존 관계가 변경된 경우 `index.md` 의 "모듈 의존 관계" 표(SSOT)만 갱신한다. (`modules/<domain>.md` 에는 module→module 엣지를 적지 않는다)
- 실행 흐름이 추가·변경된 경우 `index.md` 주요 실행 흐름 테이블 갱신
- 구현 중 발견한 암묵적 제약·주의사항이 있으면 `gotchas.md` 에 추가
...
**스킬 컨텍스트 재사용 규칙**
- 동일 세션에서 이미 받은 스킬(spec-init·spec-todo·spec-work)의 본문은 컨텍스트에 남아 있으므로, 같은 스킬을 다시 받을 목적만으로 도구를 재호출하지 않는다.
- 단 `spec_work` 는 승인 게이트 확인을 위해 반복 호출이 필요하다. 이때 이미 spec-work 스킬을 보유한 경우 `skill_loaded=true` 로 호출한다. 서버가 게이트 판정만 반환하고 스킬 본문을 생략하여 토큰을 절약한다.
...
| Cursor | 에이전트 기본 질문 방식 (전용 도구 없으면 채팅으로 선택지 제시) |
| 그 외 / 미확인 환경 | 채팅으로 선택지를 번호와 함께 제시하고 사용자 응답을 기다린다 |
...
> 아래 템플릿 첫 줄의 get_rules 참조 지시문(HTML 주석)은 커스텀 템플릿을 사용하더라도 반드시 포함한다.

```markdown
<!-- 이 spec 을 다루는 AI 는 작업 전 get_rules 를 호출하여 Spec Development Rules 를 먼저 숙지하세요. -->
# <Feature명> 요구사항
...
`PROJECT_SPEC_DIR/docs/` 폴더가 존재하는 경우, **Step 3 진행 전에 반드시 먼저 수행합니다.**
...
**0단계 — `_codebase/` 존재 여부 및 신선도 확인**
- `ai-spec/_codebase/` 폴더가 없으면 1단계를 건너뛰고 2단계로 이동한다.
- 폴더가 있으면 `last-synced.md` 를 읽어 마지막 동기화 git hash 를 확인한다.
  `git log <hash>..HEAD --oneline` 으로 이후 커밋이 존재하면 해당 모듈 정보가 오래되었을 수 있음을 인지하고, 1단계에서 얻은 정보를 그대로 신뢰하지 않고 2단계로 보완한다.

**1단계 — `_codebase/` 위키 참조 (프로젝트 공용)**
`ai-spec/_codebase/index.md` 의 모듈 맵에서 현재 작업과 관련된 도메인을 파악하고
해당 `modules/<domain>.md` 를 읽는다. 관련 파일 경로·API·패턴이 모두 파악되면 워크스페이스를 탐색하지 않는다.

**2단계 — 워크스페이스 직접 탐색 (최후 수단)**
1단계에서 정보가 부족하거나 `_codebase/` 가 없는 경우에만 최소 범위로 탐색한다.
탐색으로 새로 파악한 정보는 즉시 `ai-spec/_codebase/modules/<domain>.md` 에 반영한다.
해당 파일이 없으면 새로 생성한다. 신규 도메인이 생긴 경우 `index.md` 모듈 맵도 함께 갱신한다.

**탐색 제외 디렉토리 (모든 단계 공통)**
아래 디렉토리는 소스 코드가 아니므로 탐색에서 제외한다.
`node_modules/`, `.git/`, `dist/`, `build/`, `out/`, `coverage/`, `.next/`, `.nuxt/`, `__pycache__/`, `venv/`, `.venv/`
...
## todo.md 역할: 어떤 작업을 해야 할지 파악하기 위한 심플한 작업 목록

todo.md 는 어떤 작업이 필요한지 한눈에 파악할 수 있는 심플한 작업 목록입니다.
각 항목의 구체적인 구현 방법은 plan.md 에서 다루며, todo 항목은 작업 제목과 간단한 맥락 정도만 담습니다.
...
## [T-01] getList 메서드 추가
- 상태: [ ] TODO
- 대상 파일: `src/order/order.service.ts`
- 작업내용: product.getList 패턴을 참고해서 order.getList 메서드를 구현합니다.
```

## E2E 테스트 항목 (`T-NNE`)

구현 항목과 별개로 종단(end-to-end) 검증이 필요한 경우, 대응하는 구현 항목 번호에 `E` 접미사를 붙인 항목을 만든다.

- 형식: 구현 항목이 `T-03` 이면 그 E2E 항목은 `T-03E` 로 작성한다.
- 생성 기준: 사용자 시나리오·승인 기준이 화면 흐름이나 API 통합 동작 검증을 요구할 때만 만든다. 단위 로직만 있는 항목에는 만들지 않는다.
- 실행 순서: `T-NNE` 는 대응 구현 항목 `T-NN` 이 `[x] 완료` 된 후에만 진행한다. (spec-work 가 진입 시 선행 항목 완료 여부를 확인한다)

```markdown
## [T-03] 주문 목록 화면 구현
- 상태: [ ] TODO
- 작업내용: 주문 목록 조회 화면과 페이지네이션을 구현합니다.

## [T-03E] 주문 목록 화면 E2E 테스트
- 상태: [ ] TODO
- 작업내용: T-03 완료 후 목록 조회→페이지 이동 시나리오를 E2E로 검증합니다.
```
...
plan.md 의 내용 확인 후 아래 명령으로 응답해주세요.
  ...
  승인   → 구현을 시작합니다
  수정   → plan.md 의 User Feedback 섹션에 수정 내용 입력 후 "수정" 응답
```
...
```
1. 사용자가 plan.md 의 User Feedback 섹션에 수정 내용 입력
2. 사용자가 채팅에 "수정" 응답
3. Agent 가 User Feedback 를 읽어 계획 재수립
...
```
1. plan.md 와 `ai-spec/_codebase/modules/<domain>.md`, `ai-spec/_codebase/gotchas.md` 를 참조하면서 구현을 진행합니다.
2. 구현 중 _codebase/ 에 없는 새 심볼·파일·패턴을 발견하면 update.md 작업 목록에 메모합니다.
   (세션이 중단되어도 Step 8 에서 누락 없이 반영할 수 있도록 기록을 남깁니다.)
...
`_codebase/` 최종 업데이트 (필수) — Step 7 메모 포함 변경 전체를 반영합니다.

- 변경된 파일 구조·코드 위치·스키마·의존성을 `ai-spec/_codebase/modules/<domain>.md` 에 반영합니다.
- 모듈 간 의존 관계가 추가·변경·제거된 경우 `index.md` 의 **모듈 의존 관계** 표(SSOT)를 갱신합니다. (`modules/<domain>.md` 에는 module→module 엣지를 별도로 적지 않습니다)
- 실행 흐름이 추가·변경된 경우 `index.md` 의 **주요 실행 흐름** 테이블을 갱신합니다.
- 구현 중 코드에서 직접 확인된 암묵적 제약·주의사항을 `gotchas.md` 에 반영합니다. (위의 `gotchas.md 포맷` 섹션 작성 기준 참조)
- 신규 파일·디렉토리가 생긴 경우 `index.md` 모듈 맵도 갱신합니다.
- `last-synced.md` 를 최신 git hash 로 업데이트합니다.
