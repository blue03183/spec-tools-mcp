...
| 대상 심볼 | 모듈 | 용도 |
|-----------|------|------|
| `AuthService` | [auth](../auth.md) | 요청마다 토큰 검증 |
...
- 모듈 간 의존 관계가 변경된 경우 `modules/<domain>.md` 내부 의존성 섹션 및 `index.md` 모듈 의존 관계 테이블 갱신
...
- 모듈 간 의존 관계가 추가·변경·제거된 경우 해당 `modules/<domain>.md` 의 **내부 의존성** 섹션과 `index.md` 의 **모듈 의존 관계** 테이블을 함께 갱신합니다.
