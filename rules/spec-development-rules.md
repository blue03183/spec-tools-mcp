...
2. 구현 중 새로 발견한 심볼·파일·패턴은 발견 즉시 `ai-spec/_codebase/modules/<domain>.md` 에 반영합니다. 세션이 중단되어도 다음 세션이 최신 맥락을 이어받을 수 있도록 지연하지 않습니다.
...
Step 7 에서 즉시 반영하지 못한 내용을 포함하여 변경된 파일 구조·코드 위치·스키마·의존성을 `ai-spec/_codebase/modules/<domain>.md` 에 **최종 업데이트합니다.**
신규 파일·디렉토리가 생긴 경우 `index.md` 모듈 맵도 갱신하고, `last-synced.md` 를 최신 git hash 로 업데이트합니다.
