...
[Node.js](https://nodejs.org) v18 或更高版本（推荐 LTS）。MCP 服务器通过 `npx` 启动，`spec-init` 安装的代码库 wiki 上下文钩子也通过 `node` 运行，因此 Node.js 必须存在于 `PATH` 中。可用 `node -v` 验证。
...
如果服务器已连接，AI 将列出八个工具（`spec_init`、`spec_todo`、`spec_work`、`get_rules`、`spec_status`、`spec_handoff`、`spec_archive`、`spec_search`）或返回开发规则文档。
...
**`spec_todo`**  
规划文档准备就绪后执行。分析 `docs/` 中的文件，生成 `requirement.md` 和简洁任务列表 `todo.md`——在实现开始前充当人机共同审阅需求的检查点。
...
**`spec_status`**  
多个功能同时推进、需要全局进度总览时使用。区分显示进行中（`IN PROGRESS`）与未开始（`TODO`）的任务，并一目了然地列出所有 feature 的 todo 完成率及待审批计划，避免遗漏任何审批请求。

**`spec_handoff`**  
向队友交接工作或需要长期暂停某功能时使用。将功能目标、todo 状态和关键代码位置整合为一份文档，让下一个会话或开发者无需重新扫描代码库即可立即接手。

**`spec_archive`**  
功能完全完成后调用。将 feature 文件夹移至 `ai-spec/archive/`，保持 `projects/` 仅保留活跃工作。若存在未完成的 todo，或已有同名归档文件夹，操作将被阻止。
...
3. **运行 `spec_todo`** 分析文档并生成规范文件
   ...
   - 若包含 UI 变更，AI 会生成 `preview.html` 预览并在浏览器中打开供审阅
   - 生成简洁任务列表 `todo.md`（T-01、T-02……）
   - 需要端到端验证（界面流程、API 集成）的任务还会生成配对的 E2E 项（如 `T-01E`），仅在其实现任务（`T-01`）完成后才会执行
   - 若 `requirement.md` 已存在，分析内容将追加到现有需求下方

4. **运行 `spec_work`** 实现每个任务
   - AI 为选定任务编写 `plan.md` 并请求审批
   - 直接审阅计划文件——若需修改，在 `plan.md` 的 `User Feedback` 部分填写反馈后选择 `需要修改`
   - 输入 `审批` 或 `继续` 开始实现
   - 审批门控在 MCP 服务器层强制执行：`plan.md` 处于 `[待审批]` 状态时，`spec_work` 返回阻止提示而非实现步骤，因此在你审批前不会编写任何代码
...
8. **功能完成**后归档以保持 `projects/` 整洁
   - `spec_archive` 将文件夹移至 `ai-spec/archive/`——若有未完成的 todo 或同名归档已存在，则会阻止操作
...
│   └─ gotchas.md                  # 隐式约束与注意事项（人工编写 / 代码验证）
├─ templates/                      # （可选）自定义模板
...
└─ projects/
    └─ <feature>                   # 每个功能的项目文件夹
        ...
        ├─ preview.html            # UI 预览（包含 UI 变更时生成）
        ├─ todo.md                 # AI 生成的任务列表
        ├─ docs/                   # 原始规划文件（PDF、图片等）
        └─ <T-编号>-<摘要>/        # 每个任务的文件夹
...
按照初始安装的相同步骤操作，现有插件将被替换为最新版本。

1. 打开 **Command Palette**（`Cmd+Shift+P` / `Ctrl+Shift+P`）
2. 运行 **Chat: Install Plugin From Source**
3. 粘贴相同的 URL：`https://github.com/blue03183/spec-tools-mcp`

**基于 npx 的配置（Codex CLI、Cursor 等 IDE）**

使用 `npx -y` 的配置每次启动服务器时会自动获取最新版本，无需手动更新。

如果旧版本持续被使用，请运行以下命令刷新缓存：

```bash
npx --yes spec-tools-mcp@latest
```

**本地安装（`npm install --save-dev`）**
