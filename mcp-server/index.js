...
async function readSkillText(skillName) {
    const skillPath = path.join(SKILLS_DIR, skillName, "SKILL.md");
    const content = await cachedRead(skillPath);
    return buildPrefix("SKILL.md") + content;
}
async function readSkill(skillName) {
    return { content: [{ type: "text", text: await readSkillText(skillName) }] };
}
// ── 파일시스템 헬퍼 ──────────────────────────────────────────────────────────
function specProjectsPath() {
...
async function findPlanMd(featDir, todoId) {
    try {
...
async function readApprovalStatus(planPath) {
    try {
        const lines = (await fs.readFile(planPath, "utf-8")).split("\n");
        for (let i = 0; i < lines.length; i++) {
            if (!lines[i].includes("Approval Status"))
                continue;
...
async function findUpdateMd(featDir, todoId) {
    try {
        const entries = await fs.readdir(featDir);
        const match = entries.find(e => e.startsWith(`${todoId}-`));
        if (!match)
...
export function createServer() {
    ...
    server.tool("spec_work", "todo.md 항목을 plan.md 작성 → 승인 → 구현 → 완료 보고 순으로 실행한다.", {
        ...
        if (feature && todo) {
            ...
            if (planPath) {
                ...
                if (status === "[대기]") {
                    ...
                    return { content: [{ type: "text", text: msg + skillText }] };
                }
                if (status === "[수정]") {
        ...
        return { content: [{ type: "text", text: skillText }] };
    });
    ...
    // ── 신규 도구 ──────────────────────────────────────────────────────────────
    server.tool("spec_status", "모든 feature의 todo 진행 현황과 승인 대기 항목을 요약하여 반환한다.", {
        ...
        if (feature) {
            ...
            try {
                ...
                return { content: [{ type: "text", text: `❌ \`${SPEC_ROOT_DIR}/projects/\` 폴더를 찾을 수 없습니다.` }] };
            }
        }
        if (features.length === 0) {
            return { content: [{ type: "text", text: "진행 중인 feature가 없습니다." }] };
        }
        let report = `# Spec 현황\n\n`;
    ...
    server.tool("spec_handoff", "feature의 현재 상태를 요약한 인계 문서를 생성한다. 다른 개발자나 새 세션이 바로 작업을 이어받을 수 있도록 핵심 정보를 정리한다.", {
        feature: z.string().describe("인계할 feature 폴더명"),
    }, async ({ feature }) => {
        const featDir = path.join(specProjectsPath(), feature);
        if (!(await fs.pathExists(featDir))) {
            return { content: [{ type: "text", text: `❌ feature \`${feature}\` 폴더를 찾을 수 없습니다.` }] };
        }
    ...
    server.tool("spec_archive", "완료된 feature를 ai-spec/archive/ 로 이동하여 보관한다. 미완료 todo가 있으면 force=true 를 지정해야 한다.", {
